    ...
    class Game
    {
        public void game()
        {
            ...
            while (money != 0)
            {
                ...
                Console.WriteLine("a. Even    b. Odd    c. 0 to 18    d. 19 to 36    e. Red    \nf. Black   g. 1st 12    h. 2nd 12    i. 3rd 12   j. Number \nk. Columns    l. Street  m. 6Numbers");
                pick = (Console.ReadLine());
                pick.ToLower();
                bool check = pick == "a" || pick == "b" || pick == "c" || pick == "d" || pick == "e" || pick == "f" || pick == "g" || pick == "h" || pick == "i" || pick == "j" || pick == "k" || pick == "l" || pick == "m";
                if (check == false)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else if (pick == "k")
                        {
                            ...
                            else if (choice == 1 && (new[] { 1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34 }).Contains(ranNumber))
                            {
                                Column(ref attempts, bet, ref money, ranNumber);
                            }
                            else if (choice == 2 && (new[] { 2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35 }).Contains(ranNumber))
                            {
                                Column(ref attempts, bet, ref money, ranNumber);
                            }
                            else if (choice == 3 && (new[] { 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36 }).Contains(ranNumber))
                            {
                                Column(ref attempts, bet, ref money, ranNumber);
                            }
                            else
